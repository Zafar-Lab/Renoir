...
def create_cluster(
    ...
    if method == "hdbscan":
        ...
        labels = clusters["labels"] if isinstance(clusters, dict) else clusters
...
def downstream_analysis(
    ...
    if pdf_path is not None:
        ...
        top_n = list(get_top_n_clust_pairs(neighbscore_copy, neighbscore_df, n=n_top))
        sc.pl.heatmap(
            neighbscore_copy,
            var_names=top_n,
            groupby="leiden",
...
def spot_v_spot(
    ...
    celltype_cosine = pd.DataFrame(
        data=cosine_similarity(celltype_df.T),
        columns=list(celltype_df.columns),
        index=list(celltype_df.columns),
    )
    neighbscore_cosine = pd.DataFrame(
        data=cosine_similarity(neighbscore_df.T[list(set(top_n + de_ltpairs))]),
        columns=list(neighbscore_df.columns),
        index=list(neighbscore_df.columns),
    )
    ...
    sc.pl.heatmap(
        celltype_copy,
        var_names=celltype_copy.var_names,
        groupby="leiden",
        show=False,
        swap_axes=True,
        standard_scale="var",
    ...
    for cluster in pcs.obs.leiden.unique():
        clusterobj = sc.pl.clustermap(
            pcs[pcs.obs.leiden[pcs.obs.leiden == cluster].index, :],
            show=False,
            col_cluster=False,
        )
        clustered_spot_order += list(
    ...
    if len([x for x in pcs.var_names if not x.startswith("cluster")]) > 2:
        clusterobj = sc.pl.clustermap(
            pcs[:, [x for x in pcs.var_names if not x.startswith("cluster")]],
            show=False,
        )
        plt.close()
        reordered_var = list(
            pcs[:, [x for x in pcs.var_names if not x.startswith("cluster")]].var_names[
                clusterobj.dendrogram_col.reordered_ind
            ]
        )
        sc.pl.heatmap(
            pcs,
            var_names=reordered_var,
            groupby="leiden",
            show=False,
            swap_axes=True,
            standard_scale="var",
...
def sankeyPlot(
        ...
        links.append(
            {
                ...
                "target": lig_clust.split(":")[1],
                "value": clust_v_ligand[lig_clust],
            }
    ...
    for tar_clust in target_v_clust.keys():
        links.append(
            {
                "source": tar_clust.split(":")[1],
                "target": tar_clust.split(":")[0] + "_",
                "value": (target_v_clust[tar_clust]),
            }
        )  #  / target_v_clust_sum[tar_clust.split(':')[1]])*(clust_v_ligand_avg/target_v_clust_avg
    ...
    for pair in ltpairs:
        ...
        target = pair.split(":")[1]
        links.append(
            {"source": ligand, "target": target, "value": (temp_sum[pair])}
        )  # lt_sum[target])*max_temp})   / len(ltpair_avg.keys())) * (clust_v_ligand_avg / ligand_target_avg
            ...
            links.append(
                {
                    "source": ct,
                    "target": clust,
                    "value": celltype_avg[clust][ct] * (clust_v_ligand_avg / ct_avg),
                }
            )
    ...
    fig = go.Figure(
        data=[
            go.Sankey(
                ...
                textfont=dict(color=labelcolor, size=labelsize),
            )
        ]
    )
...
def pcs_v_neighbscore(
    ...
    for cluster, ltpairs in ltpair_clusters.items():
        print(cluster)
        pc_v_neighb = pd.DataFrame(
            data=pcs.loc[cluster,].tolist(),
            index=neighbscore.obs_names,
            columns=[cluster],
        ).T
        ...
        if clustermap:
            ...
            sc.pl.clustermap(
                sc.AnnData(pc_v_neighb_df.loc[reordered_rows, :]),
                show=False,
                row_cluster=False,
            )
            temp_fig = plt.gcf()
...
def ligand_ranking(
    ...
    ligand_score_df.insert(
        loc=0,
        column="ligand score",
        value=[ligand_ranking[x] for x in ligand_score_df.index],
    )
    ...
    g = sns.jointplot(
        ...
        y=ligand_score_df.iloc[:, 1],
        kind="hist",
        bins=(len(ligand_score_df.columns), len(ligand_score_df.index)),
    )
    g.ax_marg_y.cla()
    g.ax_marg_x.cla()
    ...
    mask = ligand_score_df.apply(
        lambda x: x if not x.name.endswith("ligand score") else 0,
    ...
    sns.heatmap(
        ...
        cbar=True,
        cmap="BuPu",
        cbar_kws=dict(use_gridspec=False, location="right", shrink=0.2, pad=0.1),
    )
    ...
    mask = ligand_score_df.apply(
        lambda x: x if x.name.endswith("ligand score") else 0,
        result_type="broadcast",
        axis=0,
    ).eq(0)
    sns.heatmap(
        ...
        cbar=True,
        cmap="RdYlBu_r",
        cbar_kws=dict(use_gridspec=False, location="left", shrink=0.15, pad=0.2),
    )
    plt.subplots_adjust(left=0.1, right=0.8, top=0.9, bottom=0.1)
    # get the current positions of the joint ax and the ax for the marginal x
    pos_joint_ax = g.ax_joint.get_position()
    ...
    for col in cols:
        g.ax_marg_y.barh(
            np.arange(0.5, len(ligand_score_df.index)),
            ct_bar_plot[col].tolist(),
            color=colors[col],
        )
    ...
    for col in cols:
        g.ax_marg_x.bar(
            np.arange(1.5, len(ligand_score_df.columns)),
            ct_bar_plot[col].tolist(),
            color=colors[col],
        )
