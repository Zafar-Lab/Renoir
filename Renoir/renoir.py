...
def get_ligand_target(ligands, targets, ST, SC, expins_genes, lr_database, celltypes):
    ...
    for index in range(len(ligands)):
        if (
            ligands[index] in SC_copy.var_names
            and ligands[index] in ST_copy.var_names
            and ligands[index] in expins_genes
        ) and (
            targets[index] in SC_copy.var_names
            and targets[index] in ST_copy.var_names
            and targets[index] in expins_genes
        ):
...
def ISM(
    ...
    ISM_result = np.full(
        (
            ligand_target_pairs.shape[0],
            ligand_target_pairs.shape[1],
            celltype_proportions.shape[0],
        ),
        -1,
        dtype=np.float64,
    )
...
def ISM_PEM(
    ...
    for celltype in range(len(celltypes)):
        ...
        scdata = np.concatenate(
            (
                scdata,
                np.transpose(
                    SC[
                        SC.obs.index[SC.obs["celltype"] == celltypes[celltype]],
                        ligand_target_list,
                    ].X.toarray()
                ),
            ),
...
def compute_neighborhood_scores(
    ...
    ST_path,
    pairs_path,
    ligand_receptor_path,
    celltype_proportions_path,
    expins_path=None,
    single_cell=False,
    use_radius=False,
    radius=0,
    ...
    if expins_path is not None:
        ...
        expins = compute_celltype_expression(
            ST.to_df()[shared_genes].to_numpy(),
            celltype_proportions,
            shared_genes,
            celltypes,
            ST.obs_names,
        ...
        for gene in tqdm(genes):
            expins_new.append(
                pd.DataFrame(
                    expins[gene].toarray(),
                    index=expins["cells"],
                    columns=expins["celltypes"],
                )
                .loc[ST.obs_names, celltypes]
                .astype("float64")
                .to_numpy()
            )
    ...
    graph = neighborhood(
        ST.obs["array_row"].tolist(),
        ST.obs["array_col"].tolist(),
        technology=use_radius,
        radius=radius,
    )
    ...
    ISM_result, ECS_result = ISM_PEM(
        SC,
        expins,
        list(ligand_target_index.keys()),
        ligand_target_pairs,
        celltypes,
        celltype_proportions,
        ligand_receptor_ct,
    ...
    neighborhood_scores = get_neighborhood_score(
        ligand_target_pairs,
        graph,
        ECS_result,
        ISM_result,
