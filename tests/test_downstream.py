...
from conftest import CELLTYPES  # noqa: F401
...
class TestCreateCluster:
    def test_pathway_mode_returns_dict(self, neighborhood_scores, msig_df):
        result = rd.create_cluster(neighborhood_scores, msig_df, method=None, pathway_thresh=1)
    ...
    def test_pairs_exist_in_var_names(self, neighborhood_scores, msig_df):
        ...
        for cluster_name, pairs in result.items():
            for pair in pairs:
                assert pair in valid, f"Cluster '{cluster_name}' has unknown pair: {pair}"
    ...
    def test_restrict_KHW(self, neighborhood_scores, msig_df):
        ...
        for key in result:
            assert any(
                key.startswith(p) for p in ("KEGG_", "HALLMARK_", "WP_", "cluster_")
...
class TestDownstreamAnalysis:
    ...
    def test_pcs_var_names_match_cluster_keys(self, neighborhood_scores, clusters):
        ...
        for varname in pcs.var_names:
            assert varname in clusters, f"pcs varname '{varname}' not in cluster keys"
...
class TestSpotVSpot:
    def test_writes_pdf(self, neighborhood_scores, celltype_adata, clusters, tmp_path):
        import os
    ...
    def test_leiden_added_to_obs(self, neighborhood_scores, celltype_adata, clusters, tmp_path):
        rd.spot_v_spot(
            neighborhood_scores,
            celltype_adata,
            resolution=0.5,
...
class TestSpotVSpotBranches:
    def test_raises_when_ltpair_clusters_none(self, neighborhood_scores, celltype_adata, tmp_path):
        with pytest.raises((TypeError, Exception)):
            rd.spot_v_spot(
                neighborhood_scores,
                celltype_adata,
